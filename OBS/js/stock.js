...
function addStock(){
  ...
  const s=getStock(); const idx=s.findIndex(x=>String(x.code).toLowerCase()===code.toLowerCase());
  if(idx>-1){ s[idx].name=name; s[idx].rate=rate; s[idx].qty = Number(s[idx].qty||0) + qty; }
  else{s.push({code,name,rate,qty});}
...
function showStock(){
  const s=getStock(); let html='<table class="table"><thead><tr><th>#</th><th>Code</th><th>Name</th><th>Rate</th><th>Qty</th><th>Action</th></tr></thead><tbody>';
  s.forEach((it,idx)=> html += `<tr><td>${idx+1}</td><td>${it.code}</td><td>${it.name}</td><td>${it.rate}</td><td>${it.qty}</td>
    <td><button onclick="editRow(${idx})">Edit</button> <button onclick="removeRow(${idx})" class="btn ghost">Remove</button></td></tr>`);
  html += '</tbody></table>'; document.getElementById('stockList').innerHTML = html;
}
